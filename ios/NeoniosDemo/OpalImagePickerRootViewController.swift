...
open class OpalImagePickerRootViewController: UIViewController, ImagePickerCollectionViewCellprotocol {
    ...
    func updateCell(indexPath: IndexPath, asset: PHAsset, assetURL: URL) {
        ...
        if self.selectedAsset.count > 0 {
            self.checkSelectedAsset(assetURL: assetURL)
        }
    ...
    func checkSelectedAsset(assetURL: URL) {
                ...
                if i < self.selectedAsset.count {
                    ...
                    PHImageManager.default().requestImageData(for: asset, options: options, resultHandler: { imageData, dataUTI, orientation, info in
                        ...
                        if info?["PHImageFileURLKey"] != nil {
                            let selectedAssetUrl = info?["PHImageFileURLKey"] as? URL
                            
                            if assetURL == selectedAssetUrl {
                                print(self.selectedAsset.count)
                                //                        if i >= self.selectedAsset.count {
    ...
    @objc private func segmentTapped(_ sender: UISegmentedControl) {
        ...
        if rightExternalCollectionViewConstraint == nil {
            let rightConstraint = externalCollectionView?.constraintEqualTo(with: self.view as Any, attribute: .right)
...
extension OpalImagePickerRootViewController: UICollectionViewDelegate {
    ...
    public func imageDeletion(indexpath: IndexPath){
        ...
        if NeonImagesHandler.singleonInstance.imagesCollection.count > 0 {
            for i in 0...NeonImagesHandler.singleonInstance.imagesCollection.count - 1 {
                let item = NeonImagesHandler.singleonInstance.imagesCollection[i]
                if imageUrl_deselect!.absoluteString.replacingOccurrences(of: "file:///", with: "") == item.getFilePath() {
                    NeonImagesHandler.singleonInstance.imagesCollection.remove(at: i)
                    print("selectedAsset count after",NeonImagesHandler.singleonInstance.imagesCollection.count)
                    break
                }
            }
        }
